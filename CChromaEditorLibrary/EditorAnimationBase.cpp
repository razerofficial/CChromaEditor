...
void EditorAnimationBase::ImportTextureImage()
{
	...
	CString szDir;
	size_t lastSlash = _mPath.find_last_of(L"/\\");
	if (lastSlash < 0)
	{
		return;
	}
	wstring path = _mPath.substr(0, lastSlash);
	...
	{
		string cstr = CT2CA(fileName);
		wstring texturePath(cstr.begin(), cstr.end());
...
void EditorAnimationBase::ImportTextureImageSequence()
{
	...
	if (dlgFile.DoModal() == IDOK)
	{
		...
		std::size_t foundSeparator = texturePath.find_last_of(L"/\\");
		std::size_t foundDot = texturePath.find_last_of(L".");
		wstring path = texturePath.substr(0, foundSeparator);
		wstring filename = texturePath.substr(foundSeparator + 1, foundDot-foundSeparator-1);
		wstring extension = texturePath.substr(foundDot);

		int index = stoi(filename);
		wstring strIndex = to_wstring(index);

		int hasExtraDigits = (int)(filename.length() - strIndex.length());

		ChromaLogger::wprintf(L" path: %s\r\n", path.c_str());
		ChromaLogger::wprintf(L" file: %s\r\n", filename.c_str());
		ChromaLogger::wprintf(L" extension: %s\r\n", extension.c_str());
		ChromaLogger::wprintf(L" index: %d\r\n", index);
		if (hasExtraDigits > 0)
		{
		...
		while (true)
		{
			...
			strIndex = to_wstring(index);
			if (hasExtraDigits)
			{
				while (strIndex.length() < filename.length())
				{
					strIndex = L"0" + strIndex;
				}
			}
			wstring importPath = (path + L"\\" + strIndex + extension);
...
void EditorAnimationBase::ReadImage(const std::wstring& texturePath, bool isAnimation)
{
...
void EditorAnimationBase::SetPath(const std::wstring& path)
{
	_mPath = path;
}
