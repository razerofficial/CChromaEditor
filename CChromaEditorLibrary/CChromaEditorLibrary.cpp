...
#define TEMP_FILE L"temp.chroma"
...
void CMainViewDlg::OpenOrCreateAnimation(const std::wstring& path)
{
	SetPath(path);
}
...
void CMainViewDlg::LoadFile()
{
	if (_mPath.empty())
	{
		ChromaLogger::fwprintf(stderr, L"LoadFile: Path cannot be empty! Using `%s` instead.\r\n", TEMP_FILE);
		SetPath(TEMP_FILE);
	}
...
{
	FILE* stream;
	int result = _wfopen_s(&stream, _mPath.c_str(), L"wb");
	if (result == 13)
	{
...
void CMainViewDlg::RefreshFrames()
{
	...
	case EChromaSDKDeviceTypeEnum::DE_1D:
		...
		frameCount = (int)_mEdit1D.GetFrames().size();
		break;
	case EChromaSDKDeviceTypeEnum::DE_2D:
		currentFrame = _mEdit2D.GetCurrentFrame();
		frameCount = (int)_mEdit2D.GetFrames().size();
		break;
	}
...
void CMainViewDlg::SetPath(const wstring& path)
{
	_mPath = path;
	UpdateWindowTitle();
}
...
BOOL CMainViewDlg::OnInitDialog()
{
	...
	{
		const wchar_t* strKey = ChromaSDKPlugin::GetInstance()->GetKeyboardChar((EChromaSDKKeyboardKey)key);
	...
	{
		const wchar_t* strLed = ChromaSDKPlugin::GetInstance()->GetMouseChar((EChromaSDKMouseLED)led);
